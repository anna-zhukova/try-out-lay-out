...
function isAllCentered(elements) {
...
function getWarnings(warnings){
  warnings.forEach(function(value) {
    switch (value) {
      ...
      message = "Elements should be kept away from the edges of the page, especially if the design is going to be printed. Having elements too close to the edge can result in elements getting cut off.";
      ...
      case WarningsEnum.opticalCenter:
      message = "Object is not in the optical center. \r\n Optical center is " + helpOpticalCenter(document.querySelectorAll(".canvas-item")[0]);
      showMessage(".btn-optcenter", "#optcenter", message);
      break;
      ...
      message = "The text with center alignment should be vertically centered.";
      ...
      case WarningsEnum.mustBeCenterAligned:
      message = "There is text with center alignment used in the design, which means all elements should be vertically centered.";
      showMessage(".btn-balance", "#balance", message);
      break;
...
function hideWarnings() {
  let buttons = document.querySelectorAll(".message-container > a:not(.btn-ratio)");
  for (i = 0; i < buttons.length; i++) {
    buttons[i].classList.add("disabled");
    buttons[i].style.color = "#DCDCDC";
  }
  let messages = document.querySelectorAll(".collapse:not(#ratio)");
...
function evaluate() {
  ...
  if (objects.length === 0) {
    ...
    let border = isOnTheBorder(objects[0]);
    if (border !== 0) {
      warnings.add(border);
    ...
    if (objects[0].classList.contains("text")) {
      ...
      // check if center alligned text is in the center of the canvas
      if (objects[0].classList.contains("center")) {
        centerMust = true;
        let centerAlignment = isCenterAligned(objects[0]);
        if (!centerAlignment) {
          warnings.add(18);
          errors.push(objects[0]);
        }
      ...
      // check if the text is big enough and readable
    ...
    if (!(objects[0].classList.contains("heading") || objects[0].classList.contains("left"))) {
...
function evaluateComposition(objects, warnings, errors) {
  ...
  let quartersContent = [[], [], [], []];
  let centerMust = false;


  // for every object on the canvas
  for (i = 0; i < objects.length; i++) {
    let border = isOnTheBorder(objects[i]);
    if (border !== 0) {
      warnings.add(border);
    ...
    for (j = i + 1; j < objects.length; j++) {
      if (isOverlapping(objects[i], objects[j])) {
        ...
        // only shapes and images are overlapping which is OK
...
// returns 1 if text ot logo is too close to the canvas border
function isOnTheBorder(object) {
  let objRect = object.getBoundingClientRect();
  if (object.classList.contains("text") || object.classList.contains("logo")) {
...
// return 3 if object is not contained in the optical center area at least 90%
...
// return true if objects are overlapping
